...
def main(qubits_list, device, nshots, debug=False):
    ...
    config_path = "scripts/qml-yeast_class_4q/exp_1_4Q_1L_yeast_best.json"
    logger.info("Loading experiment configuration from %s", config_path)
    data = load_data(config_path)
    
    # Extract circuit parameters
    args = data['args']
    num_qubits = args.get('num_qubits', 4)  # default to 4 qubits
    num_layers = args.get('num_layers', 1)  # default to 1 layer
