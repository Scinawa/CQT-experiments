...
def make_mask_tensor(nlayers=1):
    ...
    n_out_features_per_layer=2
    n_out_features = nlayers * n_out_features_per_layer
    mask_tensor = torch.tensor([[0. if j==int(np.mod(i+1,n_in_features)) else 1. for j in range (n_in_features)] for i in range(n_out_features)])
...
def load_trained_data(file_path):
    ...
    trained_weights = np.array([np.diag(pair) for pair in trained_weights.reshape([10,2])])
...
def main(backend, qubit_id, nlayers, lr, epochs, nshots, grid, num_test_samples, seed, gpu, load_and_test):
    nqubits = 1
    ...
    circuit_structure.extend([trainable_circuit(nqubits, entanglement=False, density_matrix=False)])
    ...
    with open(os.path.join(output_dir, f"results_reuploading_classifier.json"), "w") as file:
        json.dump(report_data, file, indent=4)
    with open(os.path.join(output_dir, f"data_reuploading_classifier.json"), "w") as file:
        json.dump(static_meta_data, file, indent=4)

    # Load from results.json and generate plots
    with open(os.path.join(output_dir, f"results_reuploading_classifier.json"), "r") as file:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--backend", type=str, default="numpy", help="Backend to use (default: numpy). Use numpy for local simulation."
    )
    #parser.add_argument(
     #   "--nqubits", type=int, default=1, help="Number of qubits (default: 1)"
    #)
    parser.add_argument(
        "--qubit_id", type=int, default=9, help="Qubit ID (default: 9)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--grid", type=int, default=11, help="Number of grid points along both paramter directions for generating training dataset (default: 11)",
    )
    parser.add_argument(
        "--num_test_samples", type=int, default=100, help="Number of test samples (default: 100)",
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed value for initializing trainable circuit parameters (default: 42)"
    )
    parser.add_argument(
        "--gpu", type=int, default=None, help="Option to run job on GPU when backend is qiboml. Enter GPU cuda ID (default: None)"
    )
    parser.add_argument(
        ...
        "--load_and_test", type=bool, default=True, help="Option to load trained weights into model (nlayers=10 only) instead of training (test: True)"
    )
